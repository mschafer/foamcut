...
void USBIOClkConfig(void) {
    ...
    /* Enable Timer32_1, IOCON, and USB blocks */SCB_SYSAHBCLKCTRL |=
            (SCB_SYSAHBCLKCTRL_CT32B1 | SCB_SYSAHBCLKCTRL_IOCON
    ...
    while (!(SCB_USBPLLSTAT & SCB_USBPLLSTAT_LOCK))
...
void USB_Reset(void) {
    ...
    /* Enable all eight(8) EPs, note: EP won't be ready until it's
     ...
     to the command engine. */USB_DEVINTEN = DEV_STAT_INT | (0xFF << 1)
...
uint32_t USB_EPBufState(uint32_t EPNum) {
    uint32_t val;
    WrCmd(CMD_SEL_EP(EPAdr(EPNum)));
    val = RdCmdDat(DAT_SEL_EP(EPAdr(EPNum)));
    return val;
}
 ...
 *  0..3: address
 *  7   : direction
 * \param pData Pointer to data buffer which must hold at least 64 bytes for a bulk
 * endpoint and 512 for an isochronous endpoint.
...
uint32_t USB_ReadEP(uint32_t EPNum, uint8_t *pData) {
    ...
    if ((EPNum & 0x0F) != 0x04) { /* Non-Isochronous Endpoint */
        WrCmdEP(EPNum, CMD_CLR_BUF);
    }
 ...
 * \todo sending an empty packet requires writing to USB_TXDATA once
 */
uint32_t USB_WriteEP(uint32_t EPNum, const uint8_t *pData, uint32_t cnt) {
    uint32_t n;

    // max transfer is 512 for isochronous and 64 for bulk
    if ((EPNum & 0x0F) == 4) {
        cnt = cnt > 512 ? 512 : cnt;
    } else {
        cnt = cnt > 64 ? 64 : cnt;
